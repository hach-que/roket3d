...
namespace cAudio
{
    ...
    IAudio* cAudioManager::createFromFile(const std::string& identifier,const std::string& file,bool stream)
    {
        if(stream){
            ...
            if(source->isValid())
            {
				...
				//std::cout << "CAUDIO: file extension is " << ext << "." << std::endl;
				//std::cout << "CAUDIO: total decoders available is " << decodermap.size() << "." << std::endl;
                std::map<std::string, IAudioDecoderFactory*>::iterator it = decodermap.find(ext);
                if(it == decodermap.end())
                {
                    delete source;
					//std::cout << "CAUDIO: it == decodermap.end() (createFromFile)" << std::endl;
                    return NULL;
                }
                IAudioDecoder* decoder = it->second->CreateAudioDecoder(source);
                IAudio* audio = new cAudio(decoder);
                audiomap[identifier] = audio;
				//std::cout << "CAUDIO: return audio (createFromFile)" << std::endl;
                return audio;
            }
            else
            {
                delete source;
				//std::cout << "CAUDIO: source->isValid() (createFromFile)" << std::endl;
                return NULL;
            }
        }
        else
        {
			...
			//std::cout << "CAUDIO: file extension is " << getExt(file) << "." << std::endl;
			//std::cout << "CAUDIO: total decoders available is " << decodermap.size() << "." << std::endl;
            delete[]tempbuf;
            delete tempsource;
			//std::cout << "CAUDIO: return guy (createFromFile)" << std::endl;
            return guy;
        }
    }
    ...
    {
		//std::cout << "CAUDIO: registering factory for " << extension << "." << std::endl;
