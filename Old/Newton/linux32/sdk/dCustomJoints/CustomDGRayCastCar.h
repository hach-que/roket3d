...
//		dFloat m_radiusInv;		
		...
		dFloat m_raycastLength;            // max raycast length
		dFloat m_suspensionLimit;          // max Limit 0 egal no limit higher is limited...
        dFloat m_tireWidth;
	...
	void AddSingleSuspensionTire (void* userData, const dVector& localPosition, 
								  dFloat mass, dFloat radius, dFloat with, dFloat friction, dFloat suspensionLimit, dFloat springConst, dFloat springDamper,
								  int castMode);

    // only use for debug from the dll client app side, it's not the real implementation
	// Some stuff is really wrong on it.
	void SetVehicleVar (dFloat chassisRotationLimit, dFloat defaultEngineForce, dFloat defaultSteerForce);
