...
void BasicBoxStacks (SceneManager& system)
{
	...
	collision = NewtonCreateBox (system.m_world, size.m_x, size.m_y, size.m_z, NULL); 
...
void BasicCylinderStacks (SceneManager& system)
{
	...
	collision = NewtonCreateCylinder (system.m_world, size.m_x, size.m_y, &matrix[0][0]); 
	BasicCollisionStacks (system, collision, 10);
	NewtonReleaseCollision (system.m_world, collision);
}
...
void BasicConvexStacks (SceneManager& system)
{
	...
	collision = NewtonCreateConvexHull (system.m_world, pointsCount, &convexShape[0].m_x, sizeof (dVector), 0.01f, NULL); 
