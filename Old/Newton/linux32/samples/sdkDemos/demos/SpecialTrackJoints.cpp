...
//				
				...
				if (contactCount > 2) {
					// project the contact to teh bounday of teh conve hull o fteh trhread foot ptint 
					dFloat maxDist;
					dFloat minDist;
					void* minContact;
					void* maxContact;
...
//xxx = NewtonWorldGetConstraintCount(world);
	...
	class FrictionTrankThreaSuspention: public CustomSlidingContact
	{
		...
		void SubmitConstrainst (dFloat timestep, int threadIndex)
		{
			...
			CustomSlidingContact::SubmitConstrainst (timestep, threadIndex);
		}
		...
		:RenderPrimitive(matrix, NULL)
	{
		...
		// create teh main body physic object
...
*/
	...
	NewtonBody* CreateBodyPart (dBone* bone, NewtonWorld* nWorld, dFloat mass) const
	{
		...
		collision = NewtonCreateConvexHull (nWorld, geomtry->m_vertexCount, geomtry->m_vertex, 3 * sizeof (dFloat), 0.1f, NULL);
...
//	NewtonBody* floor; 
	...
	for (int x = 0; x < 3; x ++) {
		for (int z = 0; z < 3; z ++) {
			...
			new FrictionTrank ("m1a1.mdl", &system, system.m_world, matrix, defaultID, tracksID);
		}
	}
}
