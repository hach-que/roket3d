...
//		 joint = (Custom6DOF*)CreateCustomJoint6DOF (&pinAndPivot[0][0], &pinAndPivot[0][0], bar, NULL);
...
#else
		collision = NewtonCreateCylinder (nWorld, beatSize.m_y, beatSize.m_x, NULL); 
#endif
...
static void AddHangingBridge (dVector position, NewtonWorld* nWorld, SceneManager* system, dInt32 linksCount)
{
	...
	boxCollision = NewtonCreateBox (nWorld, boxSize.m_x, boxSize.m_y, boxSize.m_z, NULL); 
	...
	linkCollision = NewtonCreateBox (nWorld, linkSize.m_x, linkSize.m_y, linkSize.m_z, NULL); 
...
#else
		collision = NewtonCreateCylinder (nWorld, radius, witdh, NULL); 
#endif
...
#else
		collision = NewtonCreateCylinder (nWorld, smallRadius, witdh, NULL); 
#endif
...
class ViseWornGear: public CustomWormGear
{
	...
	void SubmitConstrainst (dFloat timestep, int threadIndex)
	{
		if (!m_myControlledSlider->JoinHitLimit()) {
			CustomWormGear::SubmitConstrainst (timestep, threadIndex);
		}
	}
...
class VisePully: public CustomPulley
{
	...
	void SubmitConstrainst (dFloat timestep, int threadIndex)
	{
		if (! (m_myControlledSlider0->JoinHitLimit() || m_myControlledSlider1->JoinHitLimit())) {
			CustomPulley::SubmitConstrainst (timestep, threadIndex);
		}
	}
...
#else
		collision = NewtonCreateCylinder (nWorld, radius, width, NULL); 
#endif
