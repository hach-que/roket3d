...
static NewtonCollision* CreateBarrelHull (SceneManager& system)
{
	...
	return NewtonCreateConvexHull (system.m_world, pointsCount, &convexShape[0].m_x, sizeof (dVector), 0.01f, NULL); 
}
...
void CreateWalls (SceneManager& system)
{
	...
	collision = NewtonCreateCylinder (system.m_world, 1.0f, 1.0f, &matrix[0][0]); 
	CreateWall (system, collision, dVector(-20.0f, 0.0f, -10.0f, 0.0f), 10);
	NewtonReleaseCollision (system.m_world, collision);

	collision = NewtonCreateBox (system.m_world, 1.0f, 1.0f, 2.0f, NULL); 
