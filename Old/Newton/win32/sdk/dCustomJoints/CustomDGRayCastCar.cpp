...
CustomDGRayCastCar::CustomDGRayCastCar(int maxTireCount, int chassisAxis, int tractionAxis, int steerAxis, int frictionAxis, const dMatrix& cordenateSytem, NewtonBody* carBody, const dVector& gravity)
	:NewtonCustomJoint(1 * maxTireCount, carBody, NULL), m_gravity (gravity)
{
	dVector com;
	dMatrix tmp;
	...
	m_chassisOmegaCorrection = 0.98f;
	// engine acceleration...
    m_engineForce = 40.0f;
	// steer turn left right force...
    m_steerForce  = 60.0f;

	m_chassisAxis = (ChassisAxis) chassisAxis;
    m_tractionAxis = (TractionAxis) tractionAxis;
    m_steerAxis = (SteerAxis) steerAxis;
    m_frictionAxis = (FrictionAxis) frictionAxis;
...
void CustomDGRayCastCar::SetVehicleVar (dFloat chassisOmegaCorrection, dFloat engineForce, dFloat steerForce)
{
  m_chassisOmegaCorrection = chassisOmegaCorrection;
...
dMatrix CustomDGRayCastCar::CalculateSuspensionMatrix (int tireIndex, dFloat distance) const
{
	...
	matrix.m_posit = tire.m_harpoint - m_localFrame[int(m_chassisAxis)].Scale (distance);
	return matrix;
}
...
void CustomDGRayCastCar::CalculateTireCollision (Tire& tire, int threadIndex) const
{
  ...
  if (tire.m_tireUseConvexCastMode) {
    ...
    dVector destination (tire.m_suspensionMatrix.TransformVector(m_localFrame.m_up.Scale (-tire.m_suspensionLength)));   
	...
	dFloat hitParam2;
	dFloat hitParam3;
    // normally I can jump the else to let's place for other ray side check.
    // I separate it now because the ray need all times a push on the floor and it's possible that a object touch the tire side in same time.
	...
	// i'm not sure how I can apply the side force and tire position here...
    // tire siding ray, now separate from the floor ray pass. 
    if (NewtonWorldConvexCast (m_world, &tire.m_suspensionMatrix[0][0], &destination2[0], tire.m_shape, &hitParam2, (void*)m_body0, ConvexCastPrefilter, &info2, 1, threadIndex)){
	  if (floorcontact) {
        ...
        tire.m_posit = hitParam2 * tire.m_suspensionLength;
	  }
    ...
    if (NewtonWorldConvexCast (m_world, &tire.m_suspensionMatrix[0][0], &destination3[0], tire.m_shape, &hitParam3, (void*)m_body0, ConvexCastPrefilter, &info3, 1, threadIndex)){
	  if (floorcontact) {
        ...
        tire.m_posit = hitParam3 * tire.m_suspensionLength;
	  }	
    ...
    dVector destination (tire.m_suspensionMatrix.TransformVector(m_localFrame[int(m_chassisAxis)].Scale (-dist)));	
...
void CustomDGRayCastCar::SubmitConstrainst(dFloat timestep, int threadIndex)
{
  ...
  for (int i = 0; i < m_tiresCount; i ++) {
	...
	tire.m_tireAxelPosit = (chassisMatrix.TransformVector(tire.m_harpoint - m_localFrame[int(m_chassisAxis)].Scale (tire.m_posit)));
	...
	tire.m_longitudinalPin = (chassisMatrix[int(m_chassisAxis)] * tire.m_lateralPin);
    ...
    if (tire.m_posit < tire.m_suspensionLength)  {
	  ...
	  speed = -(relVeloc % chassisMatrix[int(m_chassisAxis)]);
	  ...
	  dVector tireForce (chassisMatrix[int(m_chassisAxis)].Scale (tire.m_tireLoad));
	  ...
	  if (isloadedtire!=0) {
        m_chassisForce += chassisMatrix[int(m_tractionAxis)].Scale(tire.m_torque*-m_engineForce); //.m_front
	    if (dAbs(m_curSpeed)!=0.0f) {
          m_chassisForce += chassisMatrix[int(m_steerAxis)].Scale(tire.m_turnforce*m_steerForce); //.m_right
	  }
...
//	  tire.m_tireJacobianRowIndex = -1;
	...
	m_curSpeed = bodyMatrix[int(m_tractionAxis)] % m_chassisVelocity;
	if (m_curSpeed>0) { 
      m_tiresRollSide = 0; 
	} else {
      m_tiresRollSide = 1;
...
void CustomDGRayCastCar::ApplyTiresTorqueVisual(Tire& tire, dFloat timestep, int threadIndex)
{
  ...
  if (dAbs(tire.m_torque) < 1.0e-3f){
	...
	if ((tireLinearSpeed!=0.0f) && (tireContactSpeed!=0.0f)) { 
...
{ // Fix the body rotation...
  m_chassisOmega = m_chassisOmega.Scale(m_chassisOmegaCorrection);
}

void CustomDGRayCastCar::ApplyTractionAndSteer(const dVector& vForce, const dVector& vPoint)
{
  dVector Torque;
  dMatrix M;
  dVector com;
