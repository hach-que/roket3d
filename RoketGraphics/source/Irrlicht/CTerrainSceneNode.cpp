...
namespace scene
{
			...
			s32 smoothFactor)
	{
		...
		float fx=0.f;
		float fx2=0.f;
		float minHeight = 999999.9f;
		float maxHeight = -999999.9f;
		for( s32 x = 0; x < TerrainData.Size; ++x )
		{
			...
			for( s32 z = 0; z < TerrainData.Size; ++z )
			{
				...
				if (vertex.Pos.Y < minHeight)
					minHeight = vertex.Pos.Y;
				if (vertex.Pos.Y > maxHeight)
					maxHeight = vertex.Pos.Y;
				vertex.Pos.Z = fz;
		...
		c8 tmp2[255];
		snprintf(tmp2, 255, "Height range is %.4f - %.4f",
			minHeight, maxHeight);
		os::Printer::log(tmp2);
	...
	IShadowVolumeSceneNode* CTerrainSceneNode::addShadowVolumeSceneNode(const IMesh* shadowMesh,
