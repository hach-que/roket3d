...
// teh pick body accelration is 5 gravities
#define MAX_PICK_ACCEL			(9.8f * 10.0f)

//static CustomPickBody* bodyPickController;
static NewtonCustomJoint* bodyPickController;
...
//					bodyPickController->SetMaxAngularFriction (MAX_PICK_ACCEL * 5.0f);
					...
					bodyPickController = CreateCustomPickBody (pickedBody, &p[0]);
					CustomPickBodySetMaxLinearFriction (bodyPickController, MAX_PICK_ACCEL);
					CustomPickBodySetMaxAngularFriction (bodyPickController, MAX_PICK_ACCEL * 5.0f);
				}
			}
		}
		...
		if (mousePickMode) {
			...
			if (bodyPickController) {
				...
				CustomPickBodySetTargetPosit (bodyPickController, &p[0]);
			}
